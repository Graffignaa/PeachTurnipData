...
function isCommandGrabbed(state) {
    return (((state >= exports.State.COMMAND_GRAB_RANGE1_START && state <= exports.State.COMMAND_GRAB_RANGE1_END) ||
        (state >= exports.State.COMMAND_GRAB_RANGE2_START && state <= exports.State.COMMAND_GRAB_RANGE2_END) ||
        (state >= exports.State.COMMAND_GRAB_RANGE3_START && state <= exports.State.COMMAND_GRAB_RANGE3_END)) &&
        state !== exports.State.BARREL_WAIT);
}
...
function handleActionCompute(state, indices, frame) {
    ...
    const incrementCount = (field, condition) => {
        ...
        ___default.update(state.playerCounts, field, (n) => n + 1);
    };
            ...
            if (state) {
                ...
                if (state.event !== null) {
                    this.emit(state.event, {
                        combo: ___default.last(this.combos),
            ...
            if (state) {
                ...
                if (terminated) {
                    this.emit("CONVERSION", {
                        combo: ___default.last(this.conversions),
                        settings: this.settings,
                    });
                }
            }
    ...
    _populateConversionTypes() {
        ...
        sortedConversions.forEach((conversions) => {
            ...
            conversions.forEach((conversion) => {
                ...
                const lastMove = ___default.last(conversion.moves);
...
function generateOverallStats(settings, inputs, stocks, conversions, playableFrameCount) {
    ...
    const conversionsByPlayer = ___default.groupBy(conversions, (conv) => { var _a; return (_a = conv.moves[0]) === null || _a === void 0 ? void 0 : _a.playerIndex; });
...
function getOpeningRatio(conversionsByPlayerByOpening, playerIndex, opponentIndices, type) {
    ...
    const opponentOpenings = ___default.flatten(opponentIndices.map((opponentIndex) => ___default.get(conversionsByPlayerByOpening, [opponentIndex, type]) || []));
...
function getBeneficialTradeRatio(conversionsByPlayerByOpening, playerIndex, opponentIndices) {
    ...
    const opponentTrades = ___default.flatten(opponentIndices.map((opponentIndex) => ___default.get(conversionsByPlayerByOpening, [opponentIndex, "trade"]) || []));
...
function isCompletedFrame(players, frame) {
    ...
    for (const player of players) {
        const playerPostFrame = ___default.get(frame, ["players", player, "post"]);
        if (!playerPostFrame) {
            return false;
        }
    }
...
class SlpParser extends events.EventEmitter {
    ...
    _handleFrameUpdate(command, payload) {
        ...
        ___default.set(this.frames, [currentFrameNumber, field, payload.playerIndex, location], payload);
        ___default.set(this.frames, [currentFrameNumber, "frame"], currentFrameNumber);
        ...
        if (settings && (!settings.slpVersion || semver.lte(settings.slpVersion, "2.2.0"))) {
            ...
            ___default.set(this.frames, [currentFrameNumber, "isTransferComplete"], false);
        }
    }
    _handleItemUpdate(payload) {
        ...
        ___default.set(this.frames, [currentFrameNumber, "items"], items);
    }
    _handleFrameBookend(payload) {
        const latestFinalizedFrame = payload.latestFinalizedFrame;
        const currentFrameNumber = payload.frame;
        ___default.set(this.frames, [currentFrameNumber, "isTransferComplete"], true);
